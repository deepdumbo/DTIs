...
function [OUT,varargout]=pso_Trelea_vectorized(SD,SD1,SD2,SD3,SP,SP1,SP2,DP,functname,D,varargin)
...
% PSO PARAMETERS
if nargin == 10      % only specified functname and D
   VRmin=ones(D,1)*-100; 
   VRmax=ones(D,1)*100;    
   VR=[VRmin,VRmax];
   minmax = 0;
   ...
   mv = 4;
   plotfcn='goplotpso';   
elseif nargin == 11  % specified functname, D, and mv
   VRmin=ones(D,1)*-100; 
   VRmax=ones(D,1)*100;    
   VR=[VRmin,VRmax];
   minmax = 0;
   ...
   P = [];
   plotfcn='goplotpso';   
elseif nargin == 12  % specified functname, D, mv, Varrange
   mv=varargin{1};
   if isnan(mv)
       mv=4;
   end
   ...
   P = [];
   plotfcn='goplotpso';   
elseif nargin == 13  % Functname, D, mv, Varrange, and minmax
   mv=varargin{1};
   if isnan(mv)
       mv=4;
   end    
   ...
   P = [];
   plotfcn='goplotpso';
elseif nargin == 14  % Functname, D, mv, Varrange, minmax, and psoparams
   mv=varargin{1};
   if isnan(mv)
       mv=4;
   end    
   ...
   plotfcn='goplotpso';   
elseif nargin == 15  % Functname, D, mv, Varrange, minmax, and psoparams, plotfcn
   mv=varargin{1};
   if isnan(mv)
       mv=4;
   end    
   ...
   P = varargin{4}; % psoparams
   plotfcn = varargin{5}; 
elseif nargin == 16  % Functname, D, mv, Varrange, minmax, and psoparams, plotfcn, PSOseedValue
   mv=varargin{1};
   if isnan(mv)
       mv=4;
   end    
...
% VECTORIZE THIS, or at least vectorize cost funct call 
 out = feval(functname,pos,SD,SD1,SD2,SD3,SP,SP1,SP2,DP);  % returns column of cost values (1 for each particle)
%---------------------------
...
for i=1:me  % start epoch loop (iterations)
     ...
     out        = feval(functname,[pos;gbest],SD,SD1,SD2,SD3,SP,SP1,SP2,DP);
     outbestval = out(end,:);
     out        = out(1:end-1,:);
